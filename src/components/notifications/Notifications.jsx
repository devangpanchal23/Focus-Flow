        ...
        <div className="h-full flex flex-col p-6 max-w-4xl mx-auto">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                {loading ? (
                    ...
                    notifications.map(n => (
                        <div key={n._id} className={`p-5 rounded-2xl border transition-all ${n.isRead ? 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700' : 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-500/30'}`}>
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex-1">
                                    <h3 className={`font-semibold ${n.isRead ? 'text-slate-700 dark:text-slate-300' : 'text-indigo-900 dark:text-indigo-300'}`}>
                                        {n.title}
                                    </h3>
                                    <p className={`mt-1 text-sm ${n.isRead ? 'text-slate-500 dark:text-slate-400' : 'text-indigo-700 dark:text-indigo-400'}`}>
                                        {n.message}
                                    </p>
